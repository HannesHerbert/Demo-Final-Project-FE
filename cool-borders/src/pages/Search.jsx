...
function Search() {
    ...
    async function fetchUsers(username) {
        try {
            let resp = await axios.get('http://localhost:8080/protected/search_user/' + username, {
              headers: {
    ...
    async function fetchNews(title) {
        try {
            let resp = await axios.get('http://localhost:8080/protected/search_post/news/' + title, {
              headers: {
                'Authorization': `Bearer ${token}`
              }  
            });
    ...
    async function fetchBlogs(title) {
        try {
            let resp = await axios.get('http://localhost:8080/protected/search_post/blogs/' + title, {
              headers: {
                'Authorization': `Bearer ${token}`
              }  
            });
